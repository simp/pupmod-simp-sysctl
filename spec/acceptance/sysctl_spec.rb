  ...
  hosts.each do |host|
    context 'file based updates' do
      ...
      it 'should have applied successfuly in the sysctl config but not live' do
        ...
        # Handle both EL7 and EL6 properly. EL6 does not support the sysctl.d
        # directory.
        on(host, 'sysctl --system || sysctl -p', :accept_all_exit_codes => true)
        result = on(host, 'sysctl -n fs.nr_open').stdout.strip
    ...
    context 'full updates' do
      ...
      context 'when given an invalid key' do
        let(:manifest) {
          <<-EOM
            sysctl::value { 'fs.this_cannot_exist':
              value => 'I like bread'
            }
          EOM
        }
